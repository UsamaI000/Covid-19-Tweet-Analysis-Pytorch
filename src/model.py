...
class RCNNSentimentNet(nn.Module):
    ...
    def forward(self, x):
        ...
        embedded_sent = self.embeddings(x)                                     # embedded_sent.shape = (seq_len, batch_size, embed_size)
        ...
        return final_out, final_out.shape
...
class LSTMSentimentNet(nn.Module):
    def __init__(self, config, vocab_size, word_embeddings):
        ...
        self.lstm = nn.LSTM(self.embedding_dim, self.hidden_dim, self.n_layers, dropout=self.drop_prob, batch_first=True, bidirectional=False)
